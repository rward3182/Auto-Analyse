...
if($localpathfound -ne 0){
    ...
    if($bproc.ExitCode -eq 1){
        ...
        if($errors -eq $null){
	        ...
	        $ObjForm.Refresh()
            Start-Sleep -Milliseconds 150
            $tproc = Start-Process -FilePath SmartVA-Analyze-cli.exe -ArgumentList "--country $tariffCountry --hiv $tariffHIV --malaria $tariffMalaria --hce $tariffHCE --freetext $tariffFreetext --figures $tariffFigures `"$tariffInput`" $tariffOutput" -NoNewWindow -PassThru -RedirectStandardError $logfiledir\TariffOutput.txt 
            $tproc
                ...
                if($tariffpredictions -ne $null){
                    ...
                    ForEach ($prediction in $tariffpredictions){
                        ...
                        ForEach ($VA in $VAs){
                            ...
                            $results += "VA ID:`t`t" + $sid + "`nAge:`t`t" + $age + "`nSex:`t`t" + $gender + "`n`nCause of Loss 1:`t" + $cod1 + "`nLikelihood:`t" + $lh1 + "`n`nCause of Death 2:`t" + $cod2 + "`nLikelihood:`t" + $lh2 + "`n`nCause of Death 3:`t" + $cod3 + "`nLikelihood:`t" + $lh3 + "`n`n" + "All Symptoms:`n" + $as_repl
                    ...
                    if($resultcount -eq 1){
                        ...
                        (new-object -ComObject wscript.shell).Popup("$allresults",0,"Smart-OneDevice Results")
                        #Ask for CoD as written on Death Cert
                        if($x -eq 0 -and $cod1 -ne "Undetermined"){
                            do{
                                $PhdUCoD = [Microsoft.VisualBasic.Interaction]::InputBox("Please enter the cause of death on the death certificate", "Cause of Death", "")
                            }until($PhdUCoD -ne "")
